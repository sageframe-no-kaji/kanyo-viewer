      ...
      <header className="border-b border-kanyo-gray-500 bg-kanyo-card">
        <div className="max-w-5xl mx-auto px-6 py-6">
          <div className="flex items-center gap-4">
            {/* Logo Circle */}
            <div className="w-16 h-16 rounded-full bg-kanyo-orange flex items-center justify-center flex-shrink-0">
              <svg viewBox="0 0 500 548.5" className="w-12 h-12 fill-white">
  ...
  return (
    ...
    >
      ...
      <div className="aspect-video bg-kanyo-gray-600 flex items-center justify-center relative overflow-hidden">
        {/* Try static image first, then display.thumbnail_url, finally fallback to icon */}
        <img
          src={`/thumbnails/${stream.id}.jpg`}
          alt={display.short_name || stream.name}
          className="w-full h-full object-cover"
          onError={(e) => {
            // Fallback to display.thumbnail_url if static image fails
        ...
        />
        {/* Fallback icon circle (hidden by default) */}
        <div className="absolute inset-0 items-center justify-center" style={{display: 'none'}}>
          <div className="w-24 h-24 rounded-full bg-kanyo-orange flex items-center justify-center">
